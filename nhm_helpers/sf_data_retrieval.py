...
def _should_retry_waterdata(exc: Exception) -> bool:
    """True for transient API errors worth retrying (rate-limit / upstream)."""
    if isinstance(exc, requests.exceptions.HTTPError):
        status = getattr(getattr(exc, "response", None), "status_code", None)
        return status in _WATERDATA_RETRYABLE_STATUSES
    if isinstance(exc, (requests.exceptions.ConnectionError, requests.exceptions.Timeout)):
        return True
    # Fallback for wrapped errors: check the string for known transient codes.
    msg = str(exc)
...
def fetch_daily_discharge_batch(
    ...
    *,
    ...
    Retries with exponential backoff on transient API errors (HTTP 429/502/503/504,
    connection/timeout). Non-transient errors return immediately with an error result.
    """
