...
def create_nhru_par_map(
    ...
    if par_sel in nhru_params:
        ...
        # Strictly increasing edges, needed by branca's StepColormap *and*
        # matplotlib's BoundaryNorm below (`bounds = par_bins`). The min/mean/max
        # interpolation collapses whenever a parameter is uniform across HRUs,
        # or its distribution is skewed enough that rounding closes a gap --
        # gwflow_coef has 340 distinct values and still collapses.
        par_bins = _step_colormap_bins(par_bins, value_min, value_max)
...
def create_annual_output_var_map(
    ...
    # Strictly increasing edges, needed by branca's StepColormap *and*
    # matplotlib's BoundaryNorm below (`bounds = var_bins`). The min/mean/max
    # interpolation collapses whenever a parameter is uniform across HRUs,
    # or its distribution is skewed enough that rounding closes a gap --
    # gwflow_coef has 340 distinct values and still collapses.
    var_bins = _step_colormap_bins(var_bins, value_min, value_max)
